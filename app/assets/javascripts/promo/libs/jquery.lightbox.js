...
{	// Create our Plugin function, with $ as the argument (we pass the jQuery object over later)
	...
	{	// Our LightboxClass definition
		...
		files: {
			...
			uncompressed: {
				js: {
					lightbox:	'/assets/javascripts/promo/libs/jquery.lightbox.js',
					colorBlend:	''
				},
				css: {
					lightbox:	'/assets/stylesheets/promo/jquery.lightbox.css'
				}
			},
			images: {
		...
		domReady: function ( )
		{
			...
			'<div id="lightbox-infoContainer-clear"></div></div></div>'+
			'</div>');
			...
			$('#lightbox-nav-btnPrev').unbind().hover(function() { // over
				...
				$(this).css({ 'background' : 'transparent url(' + $.Lightbox.files.images.prev + ') left 45% no-repeat' });
			}).click(function() {
			...
			$('#lightbox-nav-btnNext').unbind().hover(function() { // over
				...
				$(this).css({ 'background' : 'transparent url(' + $.Lightbox.files.images.next + ') right 45% no-repeat' });
			}).click(function() {
		...
		showImage: function ( image, step )
		{
			...
			switch ( step )
			{
				...
				case 4:
					...
					$('#lightbox-nav-btnPrev').css({ 'background' : 'transparent url(' + this.files.images.prev + ')  left 45% no-repeat' });
					$('#lightbox-nav-btnNext').css({ 'background' : 'transparent url(' + this.files.images.next + ')  right 45% no-repeat' });
